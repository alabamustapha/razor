...
class FPDFContratController extends Controller
{
    protected $pdf;
...
//$pdf->Cell(40,5,"",0,0,'R',true);
        ...
        {
            $this->pdf->Cell(60,5,'Cotisation TTC à percevoir pour la première période :',0,0,'R',true);
            $this->pdf->Cell(40,5,format_tarif(($devis[0]->customer_amount  + $protection_juridique) / 2).chr(128),0,0,'R',true);
            $this->pdf->Ln();
            $this->pdf->SetFont('Arial','',10);
            $this->pdf->Cell(150,5,'Soit 2 règlements de :',0,0,'R',true);
            $this->pdf->Cell(40,5,format_tarif(($devis[0]->customer_amount  + $protection_juridique) / 2).chr(128),0,0,'R',true);
        }
        else if ($devis[0]->periodicity == 4)
        {
            ...
            $this->pdf->Ln();
            $this->pdf->SetFont('Arial','',10);
            $this->pdf->Cell(150,5,'Soit 4 règlements de :',0,0,'R',true);
            $this->pdf->Cell(40,5,format_tarif(($devis[0]->customer_amount  + $protection_juridique) / 4).chr(128),0,0,'R',true);
        }
        else
        {
            ...
            $this->pdf->Ln();
            $this->pdf->SetFont('Arial','',10);
            $this->pdf->Cell(150,5,'Soit 12 règlements de :',0,0,'R',true);
            $this->pdf->Cell(40,5,format_tarif(($devis[0]->customer_amount  + $protection_juridique) / 12).chr(128),0,0,'R',true);
        }
...
//---------------------------------------------------------------------------//
        ...
        if ($devis[0]->type_product == 1)
        {
            ...
            if ($product['in_nombre_baux'] > 0) {
                $this->pdf->AddPage();
                $this->pdf->SetFont('Arial','B',10);
                $this->pdf->Cell(190,5,"(Option Protection Juridique)",0,0,'C',false);
                $this->pdf->Ln();
                $this->pdf->Ln();
                ...
                $this->pdf->Ln();
                $this->pdf->Cell(60,5,utf8_decode("Contrat n°").str_replace(utf8_decode("PR"),"PJ",display_id_contrat($devis[0]->id_contrat)).utf8_decode(" sur devis n°").$devis[0]->id."",0,0,'L',true);
                $this->pdf->SetFont('Arial','B',10);
                $this->pdf->Cell(70,5,"",0,0,'C',false);
                $this->pdf->SetFont('Arial','',9);
                ...
                $this->pdf->Cell(50,5,"Date et heure d'effet :",0,0,'L',true);
                $this->pdf->Cell(60,5,date("d/m/Y".utf8_decode( " à ")."00:00", $devis[0]->date_contract),0,0,'L',true);
                ...
                $this->pdf->Cell(50,5,utf8_decode("Echéance principale :"),0,0,'L',true);
                $this->pdf->Cell(60,5,utf8_decode(date("d/m", $devis[0]->date_contract)),0,0,'L',true);
