...
class FPDFAttestationController extends Controller
{
    protected $pdf;
    ...
    public function attestationpdf(){
...
//---------------------------------------------------------------------------//
        ...
        $this->pdf->SetFont('Arial','',10);
        $this->pdf->Cell(95,5,utf8_decode($proposant['in_risk_adresse']),0,0,'R',true);
        $this->pdf->Ln();
        $this->pdf->SetFont('Arial','B',9);
        ...
        if ($devis[0]->type_product == 1){
            ...
            $this->pdf->Cell(95,5,utf8_decode($coef_zone. ' '.$proposant['in_risk_ville']),0,0,'R',true);
            $this->pdf->Ln();
        ...
        $this->pdf->Cell(130,5,utf8_decode("".$proposant['in_customer_sigle']." ".$proposant['in_customer_prenom']." ".$proposant['in_customer_nom'].""),0,0,'L',true);
        $this->pdf->SetFont('Arial','',10);
        $this->pdf->Ln();
        ...
        $this->pdf->SetFont('Arial','B',10);
        $this->pdf->Cell(130,5,utf8_decode("".$proposant['in_risk_adresse']." ".$coef_zone.' '. $proposant['in_risk_ville'].""),0,0,'L',true);
        $this->pdf->SetFont('Arial','',10);
        $this->pdf->Ln();
